...
# Define the function for plotting the kernel parameters of each cahnnel with multiple outputs
def plot_channels(W):
    n_out = W.shape[0]
    n_in = W.shape[1]
    w_min = W.min().item()
...
# plot out the activaiton of the Convolutional layers
...
# creata a transform to resize the image and convert it to a tensor
IMAGE_SIZE = 16
...
## build a convolution al neural network class
class CNN(nn.Module):
